...
import Screeps.RoomPosition.Type (RoomPosition, roomName, x, y)
...
distance rp1 rp2 = case coord rp1 of
  Coord x1 y1 -> case coord rp2 of
    Coord x2 y2 ->
      max (abs (x1 - x2))
        (abs (y1 - y2))
  where
...
roomCoord rn
  | show rn == "sim" = Coord 0 0

roomCoord rn
  | show rn == "arena" = Coord 0 0

roomCoord roomName = case split nsRegex rn of
  [ hStr, vStr ] -> case I.fromString $ S.drop 1 hStr of
    Nothing -> err "Cannot parse horizontal part"
    Just hSteps -> case I.fromString vStr of
  ...
  vDir :: Int
  vDir
    | S.contains (S.Pattern "N") rn = -1

  vDir
    | S.contains (S.Pattern "S") rn = 1
